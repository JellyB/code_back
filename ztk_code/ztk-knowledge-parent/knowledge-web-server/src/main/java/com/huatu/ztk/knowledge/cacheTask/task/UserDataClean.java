 ...
 * 用户数据清洗实现
 * - 清洗用户收藏数据、错题数据、已完成数据
 * - 收藏数据、错题数据 会保存所有的节点数据
 * - 已完成的数据存储在SSDB中且只会保存 三级节点的数据
 * Created by lijun on 2018/9/5
 */
@Component
public class UserDataClean {
    private static final Logger log = LoggerFactory.getLogger(UserDataClean.class);
    ...
    @PostConstruct
    public void init() {
        log.info("用户行为数据自动清理开始 》》》》》》》》》》》》");
        threadPoolExecutor = new ThreadPoolExecutor(
                20, 20, 5, TimeUnit.MINUTES,
                workQueue,
                Executors.defaultThreadFactory(),
                new BlockingPolicy());
        initTheLastLevelPoint();
        Thread monitor = new Thread(() -> {
            while (true) {
                final String userId = setOperations.pop(NEED_CLEAN_USER_KEY);
                ...
                threadPoolExecutor.execute(() -> {
                    ...
                    try {
                        ...
                        setOperations.add(WRONG_CLEAN_USER_KEY, userId);
                    }
                    ...
                    long markFinish = System.currentTimeMillis() - mark;
                    log.info("处理用户 {} 已完成数据,所用时长：{}，共计时长，{}", userId, markFinish - markWrong, markFinish);
                    log.info("处理用户 {} 已完成,还剩用户量，{}", userId, setOperations.size(NEED_CLEAN_USER_KEY));
                });
            }
        });
    ...
    private void initTheLastLevelPoint() {
        if (CollectionUtils.isEmpty(THE_LAST_LEVEL_POINT)) {
            ...
            List<Knowledge> knowledgeList = poxyUtilService.getKnowledgeService().selectByExample(example);
