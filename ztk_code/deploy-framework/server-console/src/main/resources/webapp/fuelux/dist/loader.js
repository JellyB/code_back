...
define('fuelux/datagrid',['require','jquery'],function(require) {
	...
	var Datagrid = function (element, options) {
		...
		this.$tbody.on('mouseover', 'tr', function(){
			$(this).addClass('tr-mouseover');
		});
		this.$tbody.on('mouseout', 'tr', function(){
			$(this).removeClass('tr-mouseover');
		});
		
		this.$tbody.on('click', 'tr', function(){
			$(this).siblings().removeClass('tr-click');
	...
	Datagrid.prototype = {
		...
		renderData: function () {
			...
			this.options.dataSource.data(this.options.dataOptions, function (data) {
				...
				$.each(data.data, function (rowIndex, row) {
					var trow = $('<tr></tr>');
					trow.on('click', $.proxy(self.options.rowClick, this,rowIndex,row));
					even = rowIndex % 2;
                    if (even == 0) {
                        trow.attr({class: 'row-shadow'});
                    } else {
                        trow.attr({class: 'row-light'});
                    }
					rowHTML = '';
					$.each(self.columns, function (index, column) {
						if(typeof column.render!="undefined"){
							rowHTML += '<td>' + column.render(rowIndex,index,row) + '</td>';
				...
				self.stretchHeight();
				self.$element.trigger('loaded',[data.data]);
			});
		...
		reload: function (data) {
			if(typeof data!="undefined"){
				this.options.dataSource._data=data
			}
			this.options.dataOptions.pageIndex = 0;
			this.renderData();
		},
	...
	$.fn.datagrid = function (option,arg) {
		return this.each(function () {
			var $this = $(this);
	...
	Tree.prototype = {
		...
		populate: function ($el) {
			...
			this.options.dataSource.data($el.data(), function (items) {
				...
				self.$element.trigger('loaded',[items.data]);
			});
		},
