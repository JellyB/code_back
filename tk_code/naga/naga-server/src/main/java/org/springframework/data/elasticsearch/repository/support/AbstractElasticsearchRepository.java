...
public abstract class AbstractElasticsearchRepository<T, ID extends Serializable> implements
    ...
    protected volatile String oldIndexName;
    ...
    public AbstractElasticsearchRepository(ElasticsearchEntityInformation<T, ID> metadata,
        ...
        this.oldIndexName = entityInformation.getIndexName();
    ...
    private IndexQuery createIndexQuery(T entity) {
        if(oldIndexName != null && !oldIndexName.equals(entityInformation.getIndexName())){
            oldIndexName = entityInformation.getIndexName();
            //如果发现索引名称已经改变，需要重新创建Mapping
            if (createIndexAndMapping()) {
                createIndex();
                putMapping();
            }
