...
public class RuleController {
    ...
    public Boolean creatRule(@Valid @RequestBody CreatRuleDto creatRuleDto, BindingResult bindingResult,
        ...
        SchoolType schoolType = rule.getSchoolType();
        if(examType.equals(ExamType.MS)&& !CourseCategory.VIDEO.equals(courseCategory)) {//面试类型
            if (null == liveCategory) {
                throw new BadRequestException("请设置面试授课形式");
            }
        }
        if(!CourseCategory.VIDEO.equals(courseCategory)){  //如果线下课或直播课
            if(null==schoolType){
                throw new BadRequestException("请设置所属分校");
            }
        }
        for(Rule check:rules){
            if(examType.equals(check.getExamType())&&courseCategory.equals(check.getCourseCategory())){//考试类型 分类相同
                if(!CourseCategory.VIDEO.equals(courseCategory)){  //如果线下课或直播课
                    if(!schoolType.equals(check.getSchoolType()) || !rule.getTeacherType().equals(check.getTeacherType())){  //分校/授课身份不同 跳过此次判断
                        continue;
                    }
                }
                if(!CourseCategory.VIDEO.equals(courseCategory)){//不为录播课
        ...
        log.info("create rule : {} -> {}",  user.getId(),rule.toString());
        return  true;
    }
    ...
    public Boolean deleteRule(@Valid @RequestBody DeleteRuleDto deleteRuleDto, BindingResult bindingResult,
        ...
        log.info("delete rule : {} -> {}",  user.getId(),rule.toString());
    ...
    public RuleVo findByid(@PathVariable Long id){
        ...
        vo.setTeacherType(rule.getTeacherType());
        return vo;
    }
    ...
    public Boolean updateRule(@Valid @RequestBody UpdateRuleDto updateRuleDto, BindingResult bindingResult,
        ...
        for(Rule check:rules){
            if(examType.equals(check.getExamType())&&courseCategory.equals(check.getCourseCategory())){//考试类型 分类相同
                if(!CourseCategory.VIDEO.equals(courseCategory)){  //如果线下课或直播课
                    if(!schoolType.equals(check.getSchoolType())){  //分校不同 跳过此次判断
                        continue;
                    }
                }
                if(examType.equals(ExamType.MS)&& !CourseCategory.VIDEO.equals(courseCategory)){ //面试类型 且不为录播课
                    if(liveCategory.equals(check.getLiveCategory())){//授课类型相同
                        throw new BadRequestException("该时间段已有"+examType.getText()+courseCategory.getText()+liveCategory.getText()+"规则设置");
                    }
                }else {//类型 分类相同 且不是面试 返回异常
        ...
        rule.setDateEndInt(endInt);
        ruleService.save(rule);
        log.info("update rule : {} -> {}",  user.getId(),rule.toString());
        return true;
    }
