...
public class SmsUtil {
	...
	public static boolean sendSms(String mobile, String body) {
		...
		try {
			Response execute = okHttpClient.newCall(request).execute();//同步调用
	...
	public static void sendMultipleSms(List<String> mobiles, String body) {
		...
		try {
			...
			log.info("delete course sendMultipleSms: {} -> {}",mobiles,body);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
