...
//	@Query(value = "select course_live.date date, course_live.time_begin time_begin, course_live.time_end time_end from course_live_teacher join course_live on course_live_teacher.course_live_id = course_live.id where course_live_teacher.teacher_id = ?1 and course_live.date = ?2 union select date, time_begin, time_end from off_record where teacher_id = ?1 and date = ?2", nativeQuery = true)
	@Query(value = "select course_live.date date, course_live.time_begin time_begin, course_live.time_end time_end from course_live_teacher join course_live on course_live_teacher.course_live_id = course_live.id where course_live_teacher.teacher_id = ?1 and course_live.date = ?2 ", nativeQuery = true)
	 ...
	 */
	@Modifying
	@Query(value = "delete from teacher_roles where teacher_id = ?1 and role_id  in (2,3,8)", nativeQuery = true)
	 ...
	 * @return 教师
	 */
	Teacher findOneByNameAndTeacherSubjectsSubjectId(String name, Long subjectId);
}
