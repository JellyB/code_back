...
public class OffRecordController {
     ...
     * 新增请假记录
     *
     * @param offRecordDto 请假记录
     * @return 请假记录
     */
    @PostMapping
    public Object createCourse(@Valid @RequestBody CreateOffRecordDto offRecordDto, BindingResult bindingResult) {
        // 校验参数是否合法
        ...
        // 校验请假时间
        boolean flag = offRecordService.validateTime(offRecordDto.getTeacherId(),
        ...
        if (flag) {
            ...
            offRecordVo.setTimeBegin(DateformatUtil.format6(offRecord.getTimeBegin()));
            offRecordVo.setTimeEnd(DateformatUtil.format6(offRecord.getTimeEnd()));
            return offRecordVo;
        } else {
            return ResponseVo.fail("请假时间已冲突,请修改后重新提交");
        }
    }
    ...
    public Page findByTeacherId(@PathVariable Long teacherId,
                                  @DateTimeFormat(pattern = "yyyy-MM-dd") Date timeBegin, @DateTimeFormat(pattern = "yyyy-MM-dd") Date timeEnd,
                                  Pageable page) {
        String timeEndString = DateformatUtil.format0(timeEnd);
        timeEndString+=" 23:59:59";
        timeEnd=DateformatUtil.parse7(timeEndString);
        List<OffRecord> offRecords = offRecordService.findByTeacherIdAndDateBetween(teacherId, timeBegin, timeEnd, page);
        ...
        return new PageImpl<>(offRecordVos, page, offRecords.size());
    }
}
