...
LOGS_DIR=/app/logs/search-web-server
if [ -z "$LOGS_DIR" ]; then
    LOGS_DIR=/app/logs/search-web-server/
fi
if [ ! -d ${LOGS_DIR} ]; then
...
case "$1" in
    start)
        ./search-web-server.jar start
    ;;
    stop)
        ./search-web-server.jar stop
    ;;
    status)
        ./search-web-server.jar status
    ;;
    restart)
        ./search-web-server.jar restart
    ;;
    dump)
        do_dump
    ;;
