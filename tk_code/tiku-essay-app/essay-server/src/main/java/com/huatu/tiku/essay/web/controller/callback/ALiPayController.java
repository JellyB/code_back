...
import com.huatu.tiku.essay.util.pay.AliPayNotify;
...
public class ALiPayController {
    ...
    public Object aLiPaySync(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        //通知返回时验证签名
        log.info("param是否为空:" + (null == params));
        if (AliPayNotify.verify(params)) {
            if ("TRADE_FINISHED".equals(trade_status)) {
                ...
                long count = userCorrectGoodsService.countByIdAndPayTypeAndBizStatus(Long.parseLong(params.get("out_trade_no").toString()), payType, EssayGoodsOrderConstant.EssayGoodsOrderBizStatusEnum.PAYED.getBizStatus());
    ...
    public void savePayReturn(Map<String, String> params) throws Exception {
        ...
        //支付类型
        payReturn.setPaymentType(params.get("payment_type").toString());
        //支付宝交易号
        ...
        //买家支付宝账号
        if (StringUtils.isNotEmpty(params.get("buyer_email"))) {
            payReturn.setBuyerEmail(params.get("buyer_email").toString());
        }
        //交易金额
